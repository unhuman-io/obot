...
# logitech 922
dev922=$(ls /sys/bus/usb/devices/1-2.3:1.0/video4linux)
gst-launch-1.0 v4l2src device="/dev/${dev922}" ! 'video/x-raw,framerate=30/1' ! nvvidconv ! omxh264enc ! 'video/x-h264, stream-format=(string)byte-stream' ! h264parse ! tee name=t ! rtph264pay mtu=1400 ! udpsink host=${udphost} port=5001 sync=false async=false t. ! queue ! splitmuxsink location=log/c922%02d.mp4 max-size-time=100000000000 -e &
# logitech brio
